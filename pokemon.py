...
class Pokemon():
    ...
    def __init__(self, pokemon_id, pokemon_name, weapon_type, health_points, attack_rating, defense_rating):
        ...
        if  isinstance(health_points, int) and 0 <= health_points <= 100:  # int in [0,100]
            self.__health_points = health_points
        else:
            if health_points < 0:  # si es un número negativo, lo consideramos 0
                    health_points = 0
                    self.__health_points = health_points
            else:
                raise TypeError("The parameter health_points must be an integer between 0 and 100.")
        
        if isinstance(attack_rating, int) and 1 <= attack_rating <= 10:  # int in [1,10]
            self.__attack_rating = attack_rating  
        else:
             raise TypeError("The parameter attack_rating must be an integer between 1 and 10.")
        
        if isinstance(defense_rating, int) and 1 <= defense_rating <= 10:  # int in [1,10]
            self.__defense_rating = defense_rating  
        else:
            raise TypeError("The parameter defense_rating must be an integer between 1 and 10.")

        # una vez ya hayamos comprobado que todos los parámetros son correctos, asignamos el id
        if isinstance(pokemon_id, int) and pokemon_id not in conj_active_ids:  # pokemon_id debe ser un entero y no haber sido usado previamente
            self.__pokemon_id = pokemon_id  # # lo hacemos atributo de instancia
            conj_active_ids.add(pokemon_id) # al instanciar un objeto de la clase Pokemon, añadimos su id al conjunto de ids activos
        else:
    ...
    #    conj_active_ids.remove(self.__pokemon_id) # quitamos el id del pokemon eliminado del conj de ids activos
