...
class PokemonWater(Pokemon):
    ...
    def __init__(self, pokemon_id, pokemon_name, weapon_type, health_points, attack_rating, defense_rating):
        # importamos el constructor de la clase madre 
        # (con un valor de 1 para el parámetro attack_rating, para que no nos salte el error, que ya lo modificamos después)
        super().__init__(pokemon_id, pokemon_name, weapon_type, health_points, 1, defense_rating)

        # y cambiamos las condiciones para el parámetro attack_rating: rango 11-20
        if attack_rating in range(11,21):
            self.set_attack_rating(attack_rating)
        else:
            raise ValueError("The parameter attack_rating must be between 11 and 20.")
