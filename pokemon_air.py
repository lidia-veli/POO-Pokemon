...
class PokemonAir(Pokemon):
    ...
    # hay también que modificar el método fight_attack porque ahora que el ataque se efectúe no depende sólo
    # de la diferencia entre el ataque y la defensa, sino también de la probabilidad de que el ataque le afecte
    # al pokemon enemigo
    ...
    def fight_attack(self, enemy):
        ...
        if not isinstance(enemy, Pokemon):  # nos aseguramos de que el enemigo es un objeto de la clase Pokemon
            ...
            if self.get_attack_rating() >= enemy.get_defense_rating(): # si nuestro ataque es mayor que su defensa, podemos atacarle
                # si podemos atacarle    
                if enemy.fight_defense( self.get_attack_rating() ) is True:
                    # el enemigo ha recibido daño
                    return True
                else: # el enemigo no recibe daño
                    return False
            else:
